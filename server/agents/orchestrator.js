...
export async function runRadar(query) {
  ...
  return {
    ...
    pipeline: [
      ...
      { agent: "current-employee-search-agent", status: context.currentEmployeeSearch ? enrichment.stats.currentEmployeeSearch.status : "disabled", detail: context.currentEmployeeSearch ? `${enrichment.stats.currentEmployeeSearch.found} current employee counts found from ${enrichment.stats.currentEmployeeSearch.urlsChecked} fetched company-owned search-result pages/PDFs.` : "Enable Claude verify top leads in the UI to run deep current-employee web search before Claude review." },
